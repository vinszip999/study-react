...
function Counter(props) {
    ...
    return (
        <div>
            ...
            <button onClick={() => setCount(count - 1)}>감소</button> <hr/>
        </div>
    )
}

const UserProfile = function (props) {
    const [userName, setUserName] = useState("아무게")
...
const MultipleStateComponent = function (props) {
    ...
    return (
        <div>
            <h1>{count}</h1>
            <button onClick={() => setCount(count + 1)}>증가</button> <hr/>
            <h1>{text}</h1>
            <button onClick={() => setText(text + "a")}>a 추가</button> <hr/>
        </div>
    )
}
...
const StateDemoComponent = function (props) {
    ...
    return (
        <div>
            ...
            }}>{state.value1}</button> <hr/>
            <br />
            <button onClick={() => {
                setState({ ...state, value2: state.value2 * 2 })
            }}>{state.value2}</button> <hr/>
        </div>
    )
}
